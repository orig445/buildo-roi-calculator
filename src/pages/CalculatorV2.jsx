...
import { ChevronLeft, Sparkles, Users, MessageCircle, Zap, Globe, Loader2 } from "lucide-react";
...
export default function CalculatorV2() {
  ...
  const [showStickyAnalyzer, setShowStickyAnalyzer] = useState(false);
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (analyzerRef.current) {
        const rect = analyzerRef.current.getBoundingClientRect();
        setShowStickyAnalyzer(rect.bottom < 0);
      }
    };
  ...
  const handleStickyAnalyze = async () => {
    if (!stickyUrl.trim()) return;
    setStickyLoading(true);
    try {
      const normalizedUrl = stickyUrl.trim().startsWith("http") ? stickyUrl.trim() : "https://" + stickyUrl.trim();
      const res = await base44.functions.invoke("analyzeWebsite", { url: normalizedUrl });
      const data = res.data;
      setMessages(Math.min(Math.max(Math.round(data.monthly_messages / 100) * 100, 100), 20000));
      setCustomers(Math.min(Math.max(Math.round(data.monthly_customers), 1), 1000));
      setDealValue(Math.min(Math.max(Math.round(data.avg_deal_value / 10) * 10, 10), 50000));
      setSiteData(data);
      trackEvent("website_scanned", "v2", "sticky_analyzer");
      // scroll back up to show results
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "#F8F6FF", fontFamily: "'Heebo', sans-serif", color: "#2d1b69" }}>
      ...
      <div style={{
        ...
        background: "white", borderTop: "2px solid #ede8ff",
        boxShadow: "0 -4px 24px rgba(90,63,168,0.12)",
        padding: "12px 16px"
      }}>
          <div style={{ maxWidth: 680, margin: "0 auto", display: "flex", gap: 10, alignItems: "center" }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#5a3fa8", whiteSpace: "nowrap", flexShrink: 0 }}>
              ✨ הכנס אתר:
            </span>
            <div style={{ position: "relative", flex: 1 }}>
              <Globe style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", width: 14, height: 14, color: "#8b7ab8" }} />
              <input
              type="text"
              placeholder="mybusiness.co.il"
              value={stickyUrl}
              ...
              disabled={stickyLoading}
              style={{
                width: "100%", paddingRight: 34, paddingLeft: 12, paddingTop: 9, paddingBottom: 9,
                border: "1.5px solid #ede8ff", borderRadius: 10, fontSize: 14,
                color: "#2d1b69", background: "#f8f6ff", outline: "none",
                fontFamily: "'Heebo', sans-serif", boxSizing: "border-box"
              }}
